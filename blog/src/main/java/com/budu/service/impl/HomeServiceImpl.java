...
//                .orderByDesc(BlogArticle::getQuantity).last("limit 6"));
        List<BlogArticle> blogArticles = articleMapper.selectList(new QueryWrapper<BlogArticle>()
                .select("quantity,title,id")
                .eq("is_publish", PUBLISH.getCode())
                .orderByDesc("quantity").last("limit 6"));
        //文章贡献度
